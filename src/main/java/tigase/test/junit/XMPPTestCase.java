	...
	public static EqualError equalElems(Element el1, Element el2) {
		...
		if (attrs != null) {
			for (String key : attrs.keySet()) {
				...
				if (!(attrs.get(key).equals("*") || atval2.equals("*")) && !attrs.get(key).equals(atval2)) {
					return new EqualError(false, "Element: " + el2.getName() + " different value for attribute: " + key + ", "
		...
		if (cdata1 != null && !cdata1.trim().isEmpty()) {
			...
			if (!cdata1.trim().equals(cdata2.trim())) {
				return new EqualError(false, "Different CData for element: " + el2.getName() + ", expected: " + cdata1
						+ ", found: " + cdata2);
