...
function db_reload_mysql() {
	...
	java -cp "jars/*" tigase.util.DBSchemaLoader -logLevel ALL -dbType mysql -dbName ${_db_name} -dbHostname localhost -dbUser ${_db_user} -dbPass ${_db_pass} -rootUser ${_root_user} -rootPass ${_root_pass} -file database/mysql-pubsub-schema-3.1.0.sql
...
function db_reload_pgsql() {
	...
	# load PubSub 3.0.0 schema	
	java -cp "jars/*" tigase.util.DBSchemaLoader -logLevel ALL -dbType postgresql -dbName ${_db_name} -dbHostname localhost -dbUser ${_db_user} -dbPass ${_db_pass} -rootUser ${_root_user} -rootPass ${_root_pass} -file database/postgresql-pubsub-schema-3.1.0.sql
...
function db_reload_derby() {
	...
	# load PubSub 3.0.0 schema	
	java -cp "jars/*" tigase.util.DBSchemaLoader -logLevel ALL -dbType derby -dbName ${tts_dir}/${_db_name} -dbHostname localhost -dbUser ${_db_user} -dbPass ${_db_pass} -rootUser ${_root_user} -rootPass ${_root_pass} -file database/derby-pubsub-schema-3.1.0.sql
