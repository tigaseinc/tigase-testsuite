...
function db_reload_pgsql() {
	...
	java -cp "jars/*" tigase.db.util.DBSchemaLoader -L ALL -T postgresql -D ${_db_name} -H localhost -U ${_db_user} -P ${_db_pass} -R ${_root_user} -A ${_root_pass}
	...
	java -cp "jars/*" tigase.db.util.DBSchemaLoader -L ALL -T postgresql -D ${_db_name} -H localhost -U ${_db_user} -P ${_db_pass} -R ${_root_user} -A ${_root_pass} --file database/postgresql-installer-post.sql
...
function db_reload_sqlserver() {
	...
	java -cp "jars/*" tigase.util.DBSchemaLoader -L ALL -T sqlserver -D ${_db_name} -H sqlserverhost -U ${_db_user} -P ${_db_pass} -R ${_root_user} -A ${_root_pass}  --query "drop database ${_db_name}"

	# create new database
	java -cp "jars/*" tigase.util.DBSchemaLoader -L ALL -T sqlserver -D ${_db_name} -H sqlserverhost -U ${_db_user} -P ${_db_pass} -R ${_root_user}  -A ${_root_pass}
...
function db_reload_derby() {
	...
	java -cp "jars/*" tigase.util.DBSchemaLoader -L ALL -T derby -D ${tts_dir}/${_db_name} -H localhost -U ${_db_user} -P ${_db_pass} -R ${_root_user} -A ${_root_pass}
